...
namespace Problem8ExcellentStudents
{
    class Problem8ExcellentStudents
    {
        static void Main(string[] args)
        {
            ...
            where (stundent.marks.Max() == 6)
            select new { FullName = string.Join(" ", stundent.FirstName, stundent.LastName), Mark = string.Join(" ", stundent.marks) };
            

            foreach (var stundent in studentsWithA)
            {
                Console.WriteLine(stundent.FullName + " " + stundent.Mark);
            }
